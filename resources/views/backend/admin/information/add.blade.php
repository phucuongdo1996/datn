                    ...
                    <div class="card">
                        ...
                        <div class="card-footer p0">
                            <div class="row m0">
                                ...
                                <div class="col-7 date-information">{{ date('Y/m/d') }}</div>
                            </div>
                        </div>
                    </div>
                 </div>

                <form id="form-information" method="POST" action="{{ route(ADMIN_MANAGE_INFORMATION_STORE) }}">
                    @csrf
                                        ...
                                        <div class="btn wrap-input-option wp100 p0 br4">
                                            <select name="category" class="option-paginate-1 btn form-control hp100 p3 p15r p15l fs14">
                                                @foreach(INFORMATION_CATEGORIES as $item)
                                                    <option class="m20r m20l" value="{{ $item }}" {{ old('category') == $item ? 'selected' : ''  }}>{{ $item }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
